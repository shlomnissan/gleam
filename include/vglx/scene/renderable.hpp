 ...
 * types, or derive from it to implement a custom drawable. Properties that are
 * shared by every drawable object, such as @ref frustum_culled, live here
 * rather than on @ref Node so they only appear where they are meaningful.
 *
 * @ingroup SceneGroup
 */
class VGLX_EXPORT Renderable : public Node {
public:
    bool frustum_culled {true}; ///< Oobject participates in view frustum culling.

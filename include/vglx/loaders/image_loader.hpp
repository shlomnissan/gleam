 ...
 * thread and returns either a shared pointer to a fully constructed @ref Image
 * or an error message.
 *
...
public:
     ...
     * succeeds a shared pointer to a fully constructed @ref Image is returned.
     * On failure an error message describing the problem is returned instead.
     *
     ...
     */
    auto Load(const fs::path& path) const -> std::expected<std::shared_ptr<Image>, std::string>;
     ...
     * Errors can be retrieved explicitly from the handle or will be reported
     * through the logger.
     *
     * @param path Filesystem path to the image file.
     */

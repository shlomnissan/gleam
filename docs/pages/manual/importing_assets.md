...
struct MyScene : public vglx::Scene {
    ...
    auto OnUpdate(float delta) -> void override {
        if (mesh != nullptr) {
            ...
            mesh->transform.Rotate(vglx::Vector3::UnitX(), rotation_speed * delta);
            mesh->transform.Rotate(vglx::Vector3::UnitY(), rotation_speed * delta);
        }
    }
};
```
...
struct MyScene : public vglx::Scene {
    MyScene(vglx::Camera* camera) {
        ...
        Add(std::move(root.value()))->transform.Rotate(vglx::Vector3::UnitY(), vglx::math::DegToRad(90.0f));
    }
};
```

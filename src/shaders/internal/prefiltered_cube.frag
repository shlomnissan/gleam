...
// alpha = roughness^2 to match the runtime BRDF convention (pbr_material.frag).
...
void main() {
    ...
    for (uint i = 0u; i < kSampleCount; ++i) {
        ...
        if (n_dot_l > 0.0) {
            // Sample a lower mip whose solid angle matches the sample PDF to
            // suppress fireflies from bright HDR texels at high roughness.
